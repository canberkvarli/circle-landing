...
const RoadmapSection = ({ showIntro }: { showIntro: boolean }) => {
  const roadmapItems = [
    ...
    {
      status: "UPCOMING",
      statusColor: "bg-spiritual-primary",
      period: "Q2 2025",
      title: "Public Launch",
      description:
    ...
    >
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-spiritual-secondary to-spiritual-primary opacity-30"></div>

          {roadmapItems.map((item, index) => (
            <motion.div
              key={index}
              className="relative mb-12 ml-12 p-6 bg-white/70 backdrop-blur-sm rounded-xl border-l-4 border-spiritual-secondary hover:shadow-lg transition-all duration-300 hover:transform hover:translate-x-2"
              ...
              whileHover={{ x: 10 }}
            >
              <div className="absolute -left-14 top-8 w-4 h-4 bg-spiritual-secondary rounded-full border-4 border-white shadow-lg"></div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-spiritual-text-muted text-sm font-medium">
                  {item.period}
                </span>
              </div>
